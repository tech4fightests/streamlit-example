...
col_img.image("image.jpeg", width=400)
...
def play_stage(stage_idx: int, stage_period: str, total_number_of_kicks: int, total_kicks: int):
    ...
    for i in range(total_number_of_kicks + 1):
        ...
        if i < total_number_of_kicks:
            ...
            overall_kick_holder.metric("Total Amount of Kicks", total_kicks)
...
if start_button:
    ...
    with col2:  
        ...
        for stage_idx, mapping in enumerate(stage_data.items()):
            play_stage(stage_idx=stage_idx ,stage_period=mapping[0], total_number_of_kicks=mapping[1], total_kicks=total_kicks)
